...
<h2 style="text-align: center; font-family: F4">결재 현황  ${count}건</h2>
<table class="w3-table-all w3-hoverable">
	...
	<table class="w3-table-all w3-hoverable" width="100%">
		<tr class="w3-white">
			<td style="text-align:center">해당문서가 없습니다.</td>
	</table>
	</c:if>
...
<c:if test="${count>0}">
	<c:if test="${startPage > bottomLine }">
		<a href="allList?pageNum=${startPage - bottomLine}">[이전]</a> 
	</c:if>
   
	<c:forEach var="i" begin="${startPage}" end="${endPage}">   
		<a href="allList?pageNum=${i}">
	...
	<c:if test="${endPage < pageCount}">
		<a href="allList?pageNum=${startPage+bottomLine}">[다음]</a>
	</c:if>   
</c:if>
</div><br>
