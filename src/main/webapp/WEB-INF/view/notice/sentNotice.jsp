	...
	<div class="listNotice" id="received">
		<h3>보낸쪽지함</h3>
			<c:if test="${count==0 }">
				...
				<tr class="w3-grey">
				<td align="center">보낸 쪽지가 없습니다.</td>
				</table>
			</c:if>
			...
			<c:if test="${count!=0 }">
				<table class="w3-table-all w3-hoverable">
		    		...
		    		<c:forEach var="notice" items="${fromList}">
					<a href="pageNum=${currentPage}"></a> 
						<tr height="30">
							...
							${notice.title}</td>
							<td align="center" width="30">${notice.toid}</td>
			...
			<div class="w3-center">
				<c:if test="${count>0 }">
					...
					<a href="sentNotice?pageNum=${startPage-bottomLine}">[이전]</a>
					</c:if>
					<c:forEach var="i" begin="${startPage }" end="${endPage}">
					<a href="sentNotice?pageNum=${i }"> 
					...
					<c:if test="${endPage<pageCount }">
						<a href="sentNotice=${startPage+bottomLine}">[다음]</a>
					</c:if>
				</c:if>
			</div>
	</div>
